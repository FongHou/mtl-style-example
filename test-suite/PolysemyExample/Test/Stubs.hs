...
runFileSystem
  ...
  => Sem (FileSystem ': eff) x -> Sem eff x
runFileSystem = interpret
  (\case
     ReadFile_ path -> do
       FS files <- input @FS
       maybe (throw $ "readFile: no such file '" <> T.unpack path <> "'")
             return
             (lookup path files)
  )
...
runClock
  ...
  => Sem (Clock ': eff) x -> Sem (State ClockState ': eff) x
